...
int path_sum_from_node(BinaryNode* node, int X) {
	int count = 0;

	//Base case
	if (node == NULL) { return 0; }
	if (node->getData() == X) { count++; }

	//Inductive step
	count += path_sum_from_node(node->getLeft(), X - node->getData());
	count += path_sum_from_node(node->getRight(), X - node->getData());

	return count;
}

int path_sum(BinaryNode* node, int X) {
	int count = 0;

	//Base case
	if (node == NULL) { return 0; }
	count += path_sum_from_node(node, X);  // 현재 노드에서 시작하는 경로의 수

	//Inductive case - 혹시나 해서 만든 부분
	// 왼쪽 서브트리에서 시작하는 경로의 수
	count += path_sum(node->getLeft(), X);

	// 오른쪽 서브트리에서 시작하는 경로의 수
	count += path_sum(node->getRight(), X);
